...
class QPCRPrimerDesigner(BasePrimerDesigner):
    ...
    def _design_probes_only(self) -> List[Probe]:
        """Probe Only Design"""
        probe_global_args = self.config.pcr_params.probe_kwargs.to_global_args(
            primer_arg=self.config.pcr_params.primer_kwargs
        )
        ...
        print(probe_seq_args)
        # 3. Global Args 병합 (Primer3의 두 번째 인자)
        ...
        run_args.update({
            ...
            "PRIMER_INTERNAL_NUM_RETURN": self.config.pcr_params.probe_kwargs.n_candidates,
        })
