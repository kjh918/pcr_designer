...
class ProbePrimerDesigner(BasePrimerDesigner):
	...
	def _design_primers_for_probe(self, probe_amp: Amplicon) -> List[Amplicon]:
		...
		# 2. 설정값 준비
		gap = self.overrides.get("min_primer_probe_distance", 3)
		target_tm = probe.tm
	...
	def _run_primer3_with_params(self, probe, gap, target_tm, min_diff, max_diff) -> List[Amplicon]:
		...
		self.run_primer3()
		print(f"Exclusion Zone: start={excl_start}, len={excl_len}")
		if not self.primer3_result: return []
	...
	def design(self) -> List[Amplicon]:
		...
		# 1. Primer3에서 나온 Raw Probe 개수 확인
		n_raw_probes = 0
		if self.primer3_result:
			n_raw_probes = int(self.primer3_result.get("PRIMER_INTERNAL_NUM_RETURNED", 0))
		print(f"   -> Raw Probes from Primer3: {n_raw_probes}")
		...
		if n_raw_probes > 0:
			if self.probe_cfg:
				...
				max_poly_g = 4; max_3prime_gc = 3

			drop_counts = {"5_g": 0, "poly_g": 0, "3_gc": 0}

			for i in range(n_raw_probes):
				seq = self.primer3_result.get(f"PRIMER_INTERNAL_{i}_SEQUENCE")
				if not seq: continue
				
				# 필터 체크
				if seq.startswith("G"): 
					drop_counts["5_g"] += 1; continue
				if "G" * (max_poly_g) in seq: 
					drop_counts["poly_g"] += 1; continue
				
				tail_5bp = seq[-5:]
				if tail_5bp.count("G") + tail_5bp.count("C") >= max_3prime_gc: 
					drop_counts["3_gc"] += 1; continue
				...
				probe = Primer(
					...
					target_start_index=self.target_start_index, target_end_index=self.target_end_index,
					binding_start_index=self.primer3_result.get(f"PRIMER_INTERNAL_{i}")[0],
					binding_end_index=self.primer3_result.get(f"PRIMER_INTERNAL_{i}")[0] + self.primer3_result.get(f"PRIMER_INTERNAL_{i}")[1]
				)
			...
			print(f"   -> Dropped by Filters: {drop_counts}")
			print(f"   -> Valid Probes Left: {len(probe_candidates)}")

		# 3. Primer Design
		print("\n[Step 2] Designing Primers for each Probe...")
		final_results = []
		success_probe_count = 0
		
		for idx, probe_amp in enumerate(probe_candidates):
			paired = self._design_primers_for_probe(probe_amp)
			if paired:
				success_probe_count += 1
				final_results.extend(paired)
			# else:
				# print(f"	  [Fail] Probe #{idx} (Tm: {probe_amp.probe.tm:.1f}) -> No Primers found")

		print(f"   -> Probes with successful Primers: {success_probe_count} / {len(probe_candidates)}")
		print(f"   -> Total Final Amplicons: {len(final_results)}")
