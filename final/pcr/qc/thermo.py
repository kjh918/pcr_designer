...
from pcr.config.schema.qc import QCCriteria
...
class ThermoChecker:
    ...
    def run(self, amplicons: List[Amplicon]) -> List[Amplicon]:
        """
        [Standard Interface] 리스트 전체를 검사하여 통과한 것만 반환합니다.
        (Batch 처리에 사용)
        """
        valid_amplicons = []
        for amp in amplicons:
            result = self._check_single(amp)
            amp.thermo_stats = result["data"]
            if result["passed"]:
                valid_amplicons.append(amp)
            else:
                amp.is_qc_pass = False
                amp.qc_log = f"Failed Thermo: {result['fail_reason']}"
        return valid_amplicons

    # 💡 [ADDED] AttributeError를 해결하기 위한 브릿지 메서드
    def check(self, amplicon: Amplicon) -> Dict[str, Any]:
        """
        [Bridge Interface] 단일 앰플리콘 검사를 수행합니다. 
        factory나 executor의 구형 호출 방식과의 호환성을 위해 추가되었습니다.
        """
        return self._check_single(amplicon)

    def _check_single(self, amplicon: Amplicon) -> Dict[str, Any]:
        """기존의 정교한 검사 로직 (그대로 유지)"""
        details = {}
        all_pass = True
        fail_reasons = []
        ...
        if not f_res["fwd_pass"]:
            all_pass = False
            fail_reasons.append(f"Fwd({f_res.get('fwd_fail_reason')})")

        r_res = self._check_oligo(amplicon.reverse, "rev")
        details.update(r_res)
        if not r_res["rev_pass"]:
            all_pass = False
            fail_reasons.append(f"Rev({r_res.get('rev_fail_reason')})")
        ...
        if amplicon.probe:
            ...
            if not p_res["probe_pass"]:
                all_pass = False
                fail_reasons.append(f"Probe({p_res.get('probe_fail_reason')})")
        else:
            details.update({"probe_hairpin_dg": 0.0, "probe_homodimer_dg": 0.0, "probe_pass": True})
        ...
        if amplicon.probe:
            ...
            if not fp_res["hetero_fp_pass"]: all_pass = False; fail_reasons.append("Hetero_FP")
            if not rp_res["hetero_rp_pass"]: all_pass = False; fail_reasons.append("Hetero_RP")
        else:
            details.update({"hetero_fp_dg": 0.0, "hetero_fp_pass": True, "hetero_rp_dg": 0.0, "hetero_rp_pass": True})
    ...
    def _check_hetero(self, s1: str, s2: str, label: str) -> Dict[str, Any]:
        ...
        return {f"{label}_dg": round(het_dg, 2), f"{label}_pass": (het_dg >= min_het)}