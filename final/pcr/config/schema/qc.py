...
class QCCriteria(BaseModel):
    ...
    min_query_coverage: float = Field(
        0.8, 
        alias="min_query_coverage", 
        description="프라이머 전체 길이 대비 매칭 비율 (0.0~1.0)"
    )
    min_identity_threshold: float = Field(
        80.0, 
        alias="min_id_threshold", 
        description="실제 증폭 위험이 있다고 판단할 최소 일치도"
    )
    end_match_tolerance: int = Field(
        1, 
        alias="end_match_tolerance", 
        description="3' 말단 결합 시 허용할 끝단 오차 (bp)"
    )
